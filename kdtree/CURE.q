...
cure.clust:{[sample;numR;com;diml;kd]
 ...
 rep:distinct sample sami:numR{[samp;idxs;nn]nn,maxIdx imax{[samp;maxMean;nn]min{[samp;nn;maxMean]sum x*x:
  samp[maxMean]-samp[nn]}[samp;nn]each maxMean}[samp;nn]each maxIdx:idxs except nn}[sample;idxs]/maxFromMean;   /get the representative points from the cluster by choosing the most spread out points
 ...
 insertCl2:$[0=count[j3];insertCl;{[rd;insertCl;n;j3]update closDist:rd n,closIdx:enlist (max (insertCl`idx)) 
 ...
 {[insertIdx;kd;x] update clustIdx:(insertIdx) from kd where initi=x,valid}[insertIdx]/[recalc;sami] 		/update these into kdtree
 }
